    ...
    ! Sanity check: bad tau0 causes 0 source k-points and idle CPUs
    if (State%tau0 <= 0._dl .or. State%tau0 /= State%tau0 .or. State%tau0 > 1e7_dl) then
        call GlobalError('InitVars: invalid tau0 (check HQIV background / hqiv_Ha table)', error_unsupported_params)
        return
    end if
        ...
        if (j<= CTrans%max_index_nonlimber) then
            do q_ix = 1, CTrans%q%npoints
                if (.not.(State%closed.and.nint(CTrans%q%points(q_ix)*State%curvature_radius)<=CTrans%ls%l(j))) then
                    ...
                    dlnk = dlnks(q_ix)
                    apowers = pows(q_ix)
                    ! HQIV covariant: super-horizon cutoff for k < 2*pi/Theta0
                    if (k_cut_hqiv > 0._dl .and. ks(q_ix) < k_cut_hqiv) then
                        hqiv_cutoff = exp(-(ks(q_ix)/k_cut_hqiv)**1.8_dl)
                        apowers = apowers * hqiv_cutoff
                    end if
