...
int input_read_parameters_general(struct file_content * pfc,
  ...
  /** 11) HQIV (Horizon-Quantized Informational Vacuum) parameters */
  /* Read */
  ...
  if (flag1 == _TRUE_){
    if ((strstr(string1,"yes") != NULL) || (strstr(string1,"y") != NULL) || 
      ...
      pba->hqiv.inertia_factor = 1.0;
      pba->hqiv.age_hqiv = 0.0;
      pba->hqiv.vorticity_growth_exponent = 0.0;
      pba->hqiv.raw_acoustic_scale = 0.0;
      
      if (pba->background_verbose > 0) {
        printf(" -> HQIV enabled with parameters:\n");
        printf("    gamma = %.4f (thermodynamic coefficient)\n", pba->hqiv.gamma_hqiv);
        printf("    alpha = %.4f (varying G exponent)\n", pba->hqiv.alpha_hqiv);
        printf("    chi   = %.4f (light-cone average scaling)\n", pba->hqiv.chi_hqiv);
...
int input_default_params(struct background *pba,
  ...
  /** 11) HQIV (Horizon-Quantized Informational Vacuum) parameters */
  /* Default: HQIV disabled */
  pba->hqiv.hqiv_on = _FALSE_;
  /* Default parameters from Paper Table 1 */
  pba->hqiv.gamma_hqiv = 0.5;      /* Thermodynamic coefficient */
