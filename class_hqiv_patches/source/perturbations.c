...
int perturbations_init(
  ...
  /* Vectors enabled for HQIV vorticity growth (Paper Section 6) */
...
int perturbations_get_k_list(
  ...
  if (ppt->has_vectors == _TRUE_) {
    ...
    /* allocate array with, for the moment, the largest possible size
       ensure minimum ~50 elements to avoid underflow when l_vector_max is small */
    {
      int n_alloc = (int)((k_max_cmb[ppt->index_md_vectors]-k_min)/k_rec/MIN(ppr->k_step_super,ppr->k_step_sub))+1;
      if (n_alloc < 50) n_alloc = 50;
      class_alloc(ppt->k[ppt->index_md_vectors],
                  n_alloc*sizeof(double),ppt->error_message);
    }
...
int perturbations_einstein(
  ...
  if (_vectors_) {
    ...
    if (ppt->gauge == newtonian) {
      ...
      /* ==================== HQIV VECTOR VORTICITY SOURCE (Paper Section 6) ====================
         Action-derived: ∂ω/∂t + (v·∇)ω ∝ (∂f/∂φ)(k × ∇φ)·ê_ω
         Dampened in plasma phase; emergent after recombination; smeared during recombination
         due to over/under-densities. */
      if (pba->hqiv.hqiv_on == _TRUE_) {
        ...
        double grad_phi_proxy = pba->hqiv.gamma_hqiv * ppw->vector_source_pi / (k2 + 1e-30);
        vorticity_source = window * df_dphi * k * grad_phi_proxy * y[ppw->pv->index_pt_V];
        ppw->pvecmetric[ppw->index_mt_V_prime] += vorticity_source;
      }
    ...
    if (ppt->gauge == synchronous) {
      ...
      /* HQIV vorticity source (synchronous gauge): dampened in plasma, emergent post-recombination */
      if (pba->hqiv.hqiv_on == _TRUE_) {
        double vorticity_source = 0.0;
        ...
        double window = 0.5 * (1.0 + tanh((tau - tau_rec) / (delta_tau + 1e-30)));
        double grad_phi_proxy = pba->hqiv.gamma_hqiv * ppw->vector_source_pi / (k2 + 1e-30);
        vorticity_source = window * df_dphi * k * grad_phi_proxy * y[ppw->pv->index_pt_hv_prime];
        ppw->pvecmetric[ppw->index_mt_hv_prime_prime] += vorticity_source;
      }
...
int perturbations_derivs(double tau,
  ...
  /* HQIV: inertia reduction factor (Paper Section 6, Eq. 12).
     Apply reduced inertia ONLY to baryons — HQIV is strictly baryon-only.
     CDM/idm/dcdm densities forced to zero in HQIV; no inertia mod needed. */
  double f_inertia = 1.0;
  double hqiv_phi = 0.0;
  double metric_euler_baryons = 0.0;  /* -∇Φ/f for baryons when HQIV baryon-only */
  ...
  if (_scalars_) {
    ...
    if (pba->has_idm == _TRUE_){
      ...
      dy[pv->index_pt_theta_idm] =
        ...
        +metric_euler
        + k2*c2_idm*delta_idm; /* idm velocity (HQIV: idm=0, no inertia mod) */
      ...
      if (pth->has_idm_dr == _TRUE_) {
        if (ppw->approx[ppw->index_ap_tca_idm_dr] == (int)tca_idm_dr_off) {
          ...
          dy[pv->index_pt_theta_idm] = 1./(1.+S_idm_dr)* dy[pv->index_pt_theta_idm] /* This is technically correct as long as idm_dr is the last interaction calculated */
            + S_idm_dr/(1.+S_idm_dr)*(k2*(delta_idr/4. - ppw->tca_shear_idm_dr) + metric_euler)
    ...
    if (pba->has_cdm == _TRUE_) {
      ...
      if (ppt->gauge == newtonian) {
        ...
        dy[pv->index_pt_theta_cdm] = - a_prime_over_a*y[pv->index_pt_theta_cdm] + metric_euler; /* cdm velocity (HQIV: cdm=0) */
      }
    ...
    if (pba->has_dcdm == _TRUE_) {
      ...
      dy[pv->index_pt_delta_dcdm] = -(y[pv->index_pt_theta_dcdm]+metric_continuity)
        - a * pba->Gamma_dcdm / k2 * metric_euler; /* dcdm density (HQIV: dcdm=0) */

      dy[pv->index_pt_theta_dcdm] = - a_prime_over_a*y[pv->index_pt_theta_dcdm] + metric_euler; /* dcdm velocity (HQIV: dcdm=0) */
    }
