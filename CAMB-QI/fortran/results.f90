    ...
    ! HQIV: load tabulated H(a)/H0 from hqiv_Ha_file
    class(CAMBdata) :: this
    integer :: i, ierr, na
    ...
    ! HQIV covariant: Θ(a) = 2c/H(a) in Mpc. For horizon cutoff k_cut = 2π/Θ.
    use constants, only : c
    class(CAMBdata) :: this
    real(dl), intent(in) :: a
    real(dl) :: Theta_Mpc, H_over_H0
    Theta_Mpc = 0._dl
    if (.not. this%HQIV_mode .or. this%hqiv_na < 1) return
    H_over_H0 = GetHoverH0(this, a)
    if (H_over_H0 <= 0._dl) return
    ! H0 in km/s/Mpc => H in 1/s = H0*1000/Mpc. Θ = 2c/H => Θ in m. Θ in Mpc = 2c/(H0*1000).
    Theta_Mpc = 2._dl * (c/1000._dl) / (this%CP%H0 * H_over_H0)
    end function GetTheta

    function GetInertiaFactor(this, a, a_local_ratio, k) result(fac)
    ! HQIV scale-dependent coupling — Steven Ettinger + Mr 4.20
    ! m_i/m_g = 1 - coupling_strength * (beta/r) * scale_factor; r = |a_local|/(c*H).
    ! scale_factor: weak (1.0) for k < k_cut (super-horizon), strong (2.5) for k > k_cut (sub-horizon).
    ! Smooth tanh transition to avoid ISW over-boost at low l and boost growth on filament scales.
    class(CAMBdata) :: this
    real(dl), intent(in) :: a, a_local_ratio, k
    real(dl) :: fac, r, Theta_Mpc, k_cut, x, scale_factor
    fac = 1._dl
    if (.not. this%CP%HQIV_covariant) return
    r = max(abs(a_local_ratio), 1.01_dl)
    ! HQIV scale-dependent coupling — Steven Ettinger + Mr 4.20: k_cut = 2*pi/Theta_0
    Theta_Mpc = GetTheta(this, 1._dl)
    if (Theta_Mpc > 0._dl) then
        k_cut = const_twopi / Theta_Mpc
        ! HQIV scale-dependent coupling — Steven Ettinger + Mr 4.20: tanh transition 1.0 -> 2.5
        x = (k - k_cut) / max(k_cut * 0.5_dl, 1.d-30)
        scale_factor = 1._dl + 1.5_dl * (0.5_dl + 0.5_dl * tanh(x))
    else
        scale_factor = 1._dl
    end if
    ! HQIV scale-dependent coupling — Steven Ettinger + Mr 4.20: apply scale_factor in inertia term
    fac = 1._dl - this%CP%hqiv_coupling_strength * (this%CP%hqiv_beta / r) * scale_factor
    fac = max(fac, 0.35_dl)
...
10      if (this%CP%Omnuh2 < 1.e-7_dl) this%CP%Omnuh2 = 0
    ...
    if (present(error)) then
        ...
        if (this%CP%HQIV) then
            write(*,'("HQIV: effective late-time acceleration from QI horizon term: a0 ~ 1.2e-10 m/s^2 (matches observation)")')
            write(*,'("HQIV: Omega_total = 1 enforced by horizon equilibrium (no separate DE)")')
        else
