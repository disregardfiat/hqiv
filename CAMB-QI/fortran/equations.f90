    ...
    if (this%CP%HQIV .and. this%hqiv_na > 0) then
        ...
        dtauda = (c/1000._dl) / (a**2 * this%CP%H0 * grhov_t)
        return
    end if
    ...
    ! HQIV covariant: modified Poisson (G(a), inertia), continuity, Euler, c_s,eff
    ...
    if (State%CP%HQIV_covariant .and. State%HQIV_mode) then
        ...
        inertia_b = GetInertiaFactor(State, a, a_local_ratio, k)
    ...
    if (EV%TightCoupling) then
        ...
        !  zeroth order approximation to vbdot + the pig term. HQIV: c_s,eff and inertia in Euler
        vbdot=(-adotoa*vb+cs2_eff*k*clxb + k/4*pb43*(clxg-2*EV%Kf(1)*pig))/(1+pb43)
        ...
        ! HQIV covariant: use c_s,eff for delta_p_b term when already set above
        if (State%CP%HQIV_covariant .and. State%HQIV_mode) then
    ...
    if (associated(EV%OutputTransfer) .or. associated(EV%OutputSources)) then
        ...
        if (associated(EV%OutputSources)) then
            ...
            ISW = 2*phidot*exptau
            ! HQIV covariant: full modified metric term (c^2 t^2 / Theta_local)' in SW/ISW line-of-sight
            if (State%CP%HQIV_covariant .and. State%HQIV_mode) then
                if (GetTheta(State,a) > 0._dl) then
                    Theta_now = GetTheta(State, a)
                    Theta_deriv = (GetTheta(State, min(a*1.00001_dl, 1._dl)) - Theta_now) / 0.00001_dl * adotoa
                    t_Mpc = tau * a
                    horizon_dot = 2._dl*t_Mpc*a*adotoa/Theta_now - t_Mpc**2 * Theta_deriv/(Theta_now**2)
                    ISW = (2*phidot + horizon_dot) * exptau
                end if
            end if
